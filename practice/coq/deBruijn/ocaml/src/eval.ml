...
| Var x ->
  Var
    (if Nat.leb k x then S x else x)
| Abs (x, t') ->
  Abs (x, (shift (S k) t'))
| App (t1, t2) ->
  App ((shift k t1), (shift k t2))
...
| [] -> false
| h::t ->
  if eqb0 h x then true else inb x t

(** val free_list :
    char list list -> char list
    list -> namelambda -> char list
    list **)
...
| Abs0 (x, t1) ->
  free_list (x::l1) l2 t1
| App0 (t1, t2) ->
  free_list l1 (free_list l1 l2 t1)
    t2
...
| Var n ->
  (match nth l n with
   ...
   | None -> None)
| Abs (x, t1) ->
  (match debruijn_to_lambda (x::l)
           t1 with
   | Some t' -> Some (Abs0 (x, t'))
   | None -> None)
| App (t1, t2) ->
  let o = debruijn_to_lambda l t1 in
  let o0 = debruijn_to_lambda l t2
  in
  (match o with
   | Some t1' ->
     (match o0 with
